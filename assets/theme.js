  ...
  var Cart = /*#__PURE__*/function () {
    ...
    _createClass(Cart, [{
      ...
      value: function _openMiniCart() {
        ...
        /* Scroll to top eanble to uncomment below */
      ...
      value: function _rerender() {
        ...
        return fetch(url, {
          ...
          content.text().then(function (html) {
            ...
            if(typeof myDiv.querySelector('form') != 'undefined' && myDiv.querySelector('form') != null){
              if (myDiv.querySelector('form').hasAttribute('data-item-count')) {
                _this2.itemCount = parseInt(myDiv.querySelector('form').getAttribute('data-item-count'));
              }
            ...
            if (window.theme.cartType !== 'page') {
              if (window.theme.pageType !== 'cart') {
                ...
                setTimeout(function(){
                  var attr = jQuery('body').attr('data-curr');
                  if (typeof attr !== 'undefined' && attr == 'EUR') {
                    jQuery('.line-item__price, .cart-recap__price-line-price, .line-item--line-price, .cart-amount-saved').each(function(){
                      if(!jQuery(this).hasClass('data-converted-true')){
                        ...
                        if($(this).hasClass('cart-amount-saved')){
                          jQuery(this).text(saveprice);
                        }

                        else if($(this).hasClass('cart-recap__price-line-price')){
                          jQuery(this).text(finalPrice + ' EUR');
                        }
                        else {
                          jQuery(this).text(finalPrice);
                        }
        ...
        if (modal) {
          ...
          this.domDelegate.on('keyup', this._onKeyPressedListener);
           setTimeout(function(){
            var attr = jQuery('body').attr('data-curr');
            if (typeof attr !== 'undefined' && attr == 'EUR') {
              jQuery('.featured-product .price,.product-label--on-sale span').each(function(){
                var text = jQuery(this).text();
                var number = Number(text.replace(/\./g, '').replace(/,/g, '.').replace(/[^0-9\.-]+/g,""));
                var finalPrice = new Intl.NumberFormat('en-US', { style: 'currency', currency: 'EUR' }).format(number);
  ...
  var ProductVariants = /*#__PURE__*/function () {
    ...
    _createClass(ProductVariants, [{
      ...
      value: function _onVariantChanged(previousVariant, newVariant) {
        ...
        this._updateAddToCartButton(newVariant, previousVariant); // 8th: update image
        
        
        this._updateImage(newVariant, previousVariant); // 9th: store availability
      ...
      value: function _updateInventory(newVariant) {
        ...
        if (newVariant['available']) {
          ...
          if (moneyback_guarantee == 'yes' || moneyback_guarantee == 'Yes' || moneyback_guarantee == 'Y' || moneyback_guarantee == 'y') {
            ...
            //jQuery('.show_moneyback_guarantee').css({"opacity": "1", "height": "auto","margin-bottom": "10px"});
          }else{
...
//             jQuery('.pro-wtolp').attr('data-tooltip', '- '+Warranty+'-year warranty on parts & labor \n - '+Warranty+'-year loaner coverage \n - '+Warranty+'-year premium technical support');
            ...
            if (buyback_guarantee == 'Y' || buyback_guarantee == 'y' || buyback_guarantee == 'Yes' || buyback_guarantee == 'yes'){
              bg.show(); /* when display change this show()*/
            }
            else {
              bg.hide();
            }
      ...
      value: function _updateDiscountLabel(newVariant, previousVariant) {
        ...
        if (!newVariant || !(newVariant['price'] < newVariant['compare_at_price']) || !newVariant['available']) {
          discountLabel.style.display = 'none';
        } else {
          // We compute the savings based on the setting
          this.element.querySelector('.product-meta .product-meta__label-list').style.display = '';
          var savings = null;
      ...
      value: function _updateAddToCartButton(newVariant) {
        ...
        if (!newVariant) {
          ...
          if (newVariant['available']) {
            ...
            jQuery(".product-form__variants .stock-av").hide();  /* added by sp */
          }
      ...
      key: "_updateImage",
      value: function _updateImage(newVariant, previousVariant) {
        if(newVariant.featured_media !== undefined){
          var newImageId = newVariant.featured_media.id,
              prevImageId = previousVariant.featured_media.id;

          this.clickEvent = new Event('click', { bubbles: true }) //Create new click event

          if(newVariant.featured_media.id !== undefined || newVariant.featured_media.id != null || newVariant.featured_media.id != ''){
            if(newImageId != prevImageId){

              if(this.element.querySelector(".product-gallery__thumbnail-list").querySelectorAll("a.product-gallery__thumbnail").length > 0){
                this.element.querySelector(".product-gallery__thumbnail-list").querySelectorAll("a.product-gallery__thumbnail").forEach(img => {
                  if(img.dataset.mediaId == newImageId){
                    img.querySelector("img").dispatchEvent(this.clickEvent);
                  }
                });
              }
      ...
      value: function _onOptionChanged(event, target) {
        ...
        var temp = iframe_src.substring(iframe_src.indexOf('&variant=')+9, iframe_src.indexOf('&isIframeEmbed'));
        var updated_src = iframe_src.replace(temp,window.location.href.substr(window.location.href.indexOf('?variant=')+9,window.location.href.length));
        ...
        jQuery('.addon-product-list li').hide();
