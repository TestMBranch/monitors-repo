...
;(function (root, factory) {
  ...
  function Plugin(element, options) {
    ...
    nav.init = function(options){
      ...
      nav.getElementsByClassName("close-menu")[0].on("click", nav.hideSubmenus("BODY"));
      ...
      if(nav.options.hideSubWhenClickOut){
        ...
        //document.on("touchstart.close-menu", function(e){
        //if((e.target, "navigation") === false){
        //nav.hideSubmenus("BODY");
        //}
        //});
...
//       }, 100 * delayMultiplyer);
    ...
    nav.hideSubmenus = function(el) {
      ...
      for(var i = submenus.length - 1; i >= 0; i--){
        ...
        if(submenus[i].previousElementSibling.getElementsByClassName("submenu-indicator").length > 0){
          ...
          jQuery('.navigation-tabs-nav-item.is-active').removeClass('is-active');
		  jQuery('.navigation-tabs-pane.is-active').removeClass('is-active');
        }
      }
    };
    ...
    var navigationTabs = function(tab) {
      ...
      for(var i = 0; i < navs.length; i++){
        navs[i].on("mouseover", function(e){
          e.preventDefault();
